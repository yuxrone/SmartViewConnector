...
// XYU update m_hRootKey with m_hKey;
	...
	BOOL CRegKey::DeleteValue(HKEY rootKey, PTSTR pName)
	{
		if (m_hKey)
			Close();
		...
		{
			lstrcpy(m_name, pName);
			if (RegDeleteValue(m_hRootKey, pName) != ERROR_SUCCESS)
			{
				return FALSE;
			}
		}
	...
	BOOL CRegKey::DeleteKey(HKEY rootKey, PTSTR pName)
	{
		if (m_hKey)
			Close();
		...
		{
			lstrcpy(m_name, pName);
//			if (RegDeleteKey(m_hRootKey, pName) != ERROR_SUCCESS)
			if (RegDeleteTree(m_hRootKey, pName) != ERROR_SUCCESS)
			{
				return FALSE;
			}
		}
	...
	void CRegKey::setRootKey(const HKEY& rootKey)
	{
		m_hRootKey = rootKey;
	}
	const HKEY& CRegKey::getRootKey() const
	{
		return m_hRootKey;
	}
